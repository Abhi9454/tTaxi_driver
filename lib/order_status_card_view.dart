...
class OrderStatusCardView extends StatelessWidget {
            ...
            return OrderInvoiceView(
                ...
                onCashPaymentReceived: () {
                  updateCurrentOrderStatus(
                      context, bloc, runMutation, OrderStatus.finished,
                      ...
                      SizedBox(
                        ...
                        child: ElevatedButton(
                            onPressed: (result?.isLoading ?? false)
                                ? null
                                : () => updateCurrentOrderStatus(context, bloc,
                                    runMutation, OrderStatus.arrived),
                            child: Text(
                      ...
                      SizedBox(
                        ...
                        child: ElevatedButton(
                            onPressed: (result?.isLoading ?? false)
                                ? null
                                : () => updateCurrentOrderStatus(context, bloc,
                                    runMutation, OrderStatus.started),
                            child:
                      ...
                      SizedBox(
                        ...
                        child: ElevatedButton(
                            onPressed: (result?.isLoading ?? false)
                                ? null
                                : () => updateCurrentOrderStatus(context, bloc,
                                    runMutation, OrderStatus.finished),
                            child: Text(
  ...
  Future<void> updateCurrentOrderStatus(BuildContext context, MainBloc bloc,
      RunMutation runMutation, OrderStatus orderStatus,
    ...
    if (result?.hasException ?? true) {
      final snackBar = SnackBar(
          // ignore: use_build_context_synchronously
          content: RidyBanner(result?.exception.toString() ?? "Unknown error",
              type: BannerType.error));
      // ignore: use_build_context_synchronously
      ScaffoldMessenger.of(context).showSnackBar(snackBar);
      return;
    }
    final response = UpdateOrderStatus$Mutation.fromJson(result!.data!);
    bloc.add(CurrentOrderUpdated(response.updateOneOrder.toJson()));
  }
}
          ...
          return Subscription(
              ...
              builder: (data) {
                ...
                return RoundedButton(
                    ...
                    onPressed: () {
                      newMessages = 0;
                      runMutation(UpdateLastSeenMessagesAtArguments(
